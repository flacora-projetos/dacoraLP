...
A RA2 foi concluída e validada em produção: migration `20260813213535`, painel funcional `main/31f393f`, POST de edição 200 e persistência após reload, sem aplicar/aprovar/enviar. **RA3 está implementada e validada localmente** na branch `codex/ra3-analises-secao`, baseada em `7e9bc67`, sem integração, Preview, produção ou migration remota. B1–B6 no catálogo, lote geral, caneta local, contexto privado, suítes/build, segurança e navegador desktop/celular/PDF estão verdes. RA3 só fecha após novo GO para migration/Preview e smoke autenticado persistente. Auth, contexto server-side, checksum, auditoria, decisão e envio permanecem separados. A introdução do relatório continua sendo o primeiro e mais importante alvo da caneta mágica;
