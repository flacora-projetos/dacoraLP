...
A **RA2 está em correção após falha comprovada no primeiro teste autenticado de produção**. O print da Karyne v9 mostrou a área quase ilegível e a API respondeu `contexto_indisponivel`: os relatórios reais de julho foram gerados antes da RA1 e não têm `analysisContext`, enquanto os testes usavam somente snapshots enriquecidos. A branch `codex/correcao-ra2-contexto-legado` projeta o mesmo contexto a partir de `dados.faixas` no servidor, sem alterar snapshot/checksum, e transforma a caneta em cartão claro destacado. O banco confirmou 96 relatórios externos/gerados compatíveis; os seis sem projeção são artefatos de voz ou mensais internos fora desta UI. `verifica:analise`, `verifica:revisao` e build completo passaram; ainda não publicado neste marco. Sem decisão ou envio. A introdução do relatório é o primeiro e mais importante alvo da caneta mágica;
