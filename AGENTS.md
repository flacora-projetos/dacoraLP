...
RA1–RA3 estão concluídas e publicadas. **A primeira fatia da RA4 está em produção no `main/6b39e6c`.** Ela deriva prontidão editorial no servidor, impede aprovação com análises obrigatórias pendentes e mantém recusa disponível. Aprovação e envio são atos separados: após o GO, a revisão oferece **Enviar agora / Voltar para a fila**; na fila, junto de `aprovado por…`, ficam **Voltar para edição / Enviar**. A migration RA4 está aplicada no Supabase; reabrir exige versão liberada corrente, sem envio e sem qualquer intenção P5, preserva auditoria e volta a mesma versão para `gerado`. O deploy Vercel fechou em `success`; painel responde 200 e APIs privadas decisão/envio/reabertura 401 sem sessão. **Desde 2026-08-18, AV — Análise Viva e Fechamento Editorial é pré-requisito obrigatório da RA5.** O contrato é: dados do mês seguem vivos; análise vale para o snapshot factual que a originou; fato novo marca revisão necessária e preserva histórico interno; cliente recebe no fechamento só dados finais + análise final aprovada; arquivar histórico é padrão e descartar exige ação explícita/auditada. **RA5 está não iniciada e bloqueada até AV1–AV4; áudio segue congelado. AV1 já está implementada e validada localmente no OpenClaw, mas a migration `0015` ainda NÃO foi aplicada no Supabase remoto e exige GO explícito; nenhum comportamento do painel mudou nesta fase.** Fontes canônicas: `OpenClaw-Dacora/docs/HANDOFF_RESULTADO_CAMPANHAS_RELATORIOS_2026-08-18.md` e `OpenClaw-Dacora/docs/HANDOFF_AV1_ANALISE_VIVA_2026-08-18.md`. A introdução do relatório continua sendo o primeiro e mais importante alvo da caneta mágica;
C1/C2/C3 são insumo subordinado, não uma publicação independente. Cada fase
