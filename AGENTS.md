...
A RA2 publicada em `main/c5e6407` **falhou no teste autenticado real**: o POST da caneta chegou ao backend, mas retornou HTTP 422 `saida_invalida`, porque o contrato ainda exigia uma saída aplicável pelo parser e mantinha o teto de 3.500 caracteres. A correção está pronta e pushada em `codex/ra2-texto-livre-provider`: `a0c2698` pede texto puro ao Sonnet, preserva todos os blocos textuais, trata JSON apenas como compatibilidade opcional e remove o teto editorial da geração e da edição. `verifica:analise`, `verifica:revisao` e o build completo foram repetidos pela sessão coordenadora e passaram. **Ainda não houve merge, deploy nem novo teste autenticado**; RA3 continua bloqueada. Auth, contexto server-side, checksum, auditoria, decisão e envio permanecem separados. A introdução do relatório é o primeiro e mais importante alvo da caneta mágica;
