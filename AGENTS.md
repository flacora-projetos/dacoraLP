...
RA1–RA3 estão concluídas e publicadas; o refino RA3 de contexto/provider está em produção em `main/992fa56`. **RA4 está em andamento na branch `codex/ra4-editorial`, sem merge/deploy/produção.** A primeira fatia deriva prontidão editorial no servidor, impede aprovação com análises obrigatórias pendentes e mantém recusa disponível. Aprovação e envio são atos separados: após o GO, a revisão oferece **Enviar agora / Voltar para a fila**; na fila, junto de `aprovado por…`, ficam **Voltar para edição / Enviar**. Reabrir exige versão liberada corrente, sem envio e sem qualquer intenção P5, preserva auditoria e volta a mesma versão para `gerado`. **RA5: áudio congelado até segunda ordem**; o áudio já existente permanece intacto e não ganha desenvolvimento novo sem GO do PO. A introdução do relatório continua sendo o primeiro e mais importante alvo da caneta mágica;
