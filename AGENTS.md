...
A **RA2 está em correção após dois defeitos comprovados no teste autenticado de produção**. O primeiro print da Karyne v9 mostrou contraste quase ilegível e `contexto_indisponivel`; `dfbb384` corrigiu ambos em produção ao projetar o contexto legado no servidor e destacar a caneta. Na primeira geração real seguinte, o Sonnet respondeu em um envelope textual que o `JSON.parse` estrito rejeitou como `saida_invalida`. A branch `codex/correcao-ra2-contexto-legado` agora normaliza JSON estrito, cercado por Markdown ou acompanhado de preâmbulo, mas continua recusando texto livre e validando todos os números contra o contexto factual. `verifica:analise`, `verifica:revisao` e build completo passaram; esta segunda correção ainda não foi publicada neste marco. Sem decisão ou envio. A introdução do relatório é o primeiro e mais importante alvo da caneta mágica;
