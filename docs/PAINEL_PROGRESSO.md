...
**Frente paralela do provider em andamento:** a branch `codex/ra-deepseek-provider`, sobre o fechamento documental RA3 `afba341`, centraliza introdução e seções no mesmo adapter: DeepSeek primário, Claude Sonnet fallback técnico e rollback por env. O commit funcional atual é `d18b468`; regressões focadas e build completo passaram. Não houve merge, produção, aprovação, envio ou DDL; o schema remoto já tem `modelo` obrigatório e privado para auditar `provider/modelo`. Contrato e evidências ficam em [`HANDOFF_PROVIDER_DEEPSEEK_ANALISES_MENSAIS_2026-08-13.md`](HANDOFF_PROVIDER_DEEPSEEK_ANALISES_MENSAIS_2026-08-13.md).
