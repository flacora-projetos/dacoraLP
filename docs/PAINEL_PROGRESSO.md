...
O estado é **produção reprovada somente na persistência da edição; correção em `codex/ra2-rpc-editar`**. `main/87eb68a` já provou em produção a geração completa (1.421 caracteres, três parágrafos), o cartão responsivo e a textarea focada/visível. Ao salvar a sugestão `c355791f-08bc-4e6f-99c2-7e57eb92abf8` com o mesmo checksum, a RPC lançou `column reference "relatorio_checksum" is ambiguous`: o ramo gerar não toca esse `SELECT`, mas editar o alcança porque o nome de retorno da função compete com a coluna não qualificada. O endpoint converteu esse erro SQL em 409 de concorrência, embora a sugestão permaneça `pronta`, sem edição ou evento. A correção prepara migration que qualifica a coluna e devolve conjunto vazio apenas para concorrência legítima; no handler, SQL/HTTP/contrato passam a ser 502 e só revisão desatualizada vira 409. Não houve migration remota, merge, deploy, nova escrita real, aprovação ou envio desta correção.
