...
O estado é **correção da persistência publicada; smoke autenticado final pendente**. A migration `20260813213535_ra2_corrigir_persistencia_edicao` foi aplicada e conferida; a fábrica está em `master/1f38597`, o painel em `main/31f393f` e o deployment Production `dpl_DrAsn5YhYmc4DtptFpTsVES3MkHS` ficou `READY`. A função mantém `security invoker`, `search_path` vazio e execução exclusiva da `service_role`; teste transacional do ramo editar passou e foi revertido, preservando a sugestão real `pronta`, sem `editada_em`. Raiz/painel deram HTTP 200 e API sem sessão 401. O navegador chegou ao login Google e aguarda autenticação do Flávio para provar `gerar → editar → salvar`. Não houve aplicar, aprovação ou envio.
