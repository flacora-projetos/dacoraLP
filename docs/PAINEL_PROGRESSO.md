...
**AV — Análise Viva e Fechamento Editorial, decisão de produto em 2026-08-18:** AV é pré-requisito obrigatório da RA5. Dados do mês devem continuar vivos; análise humana vale para o snapshot factual que a originou; fato novo marca `revisão necessária` e preserva a versão anterior como histórico interno; o cliente recebe no fechamento apenas dados finais + análise final aprovada. **AV1 já está implementada e validada localmente no OpenClaw** com fingerprint factual editorial, identidade lógica durável, histórico/contexto persistentes e RPCs testadas em Supabase local. A migration `0015_av1_analise_viva_snapshot_factual.sql` **ainda não foi aplicada no Supabase remoto** e exige GO explícito. Portanto, o painel e o runtime atuais ainda mantêm o bloqueio antigo após trabalho editorial até AV2. Fontes canônicas: `OpenClaw-Dacora/docs/HANDOFF_RESULTADO_CAMPANHAS_RELATORIOS_2026-08-18.md` e `OpenClaw-Dacora/docs/HANDOFF_AV1_ANALISE_VIVA_2026-08-18.md`.
