...
**AV4 — fechamento editorial e retenção, banco EM PRODUÇÃO e portal ainda NÃO publicado em 2026-08-18:** a aprovação final do painel passa a chamar `aprovar_e_fechar_relatorio_editorial`, ligando na mesma transação o checksum do documento, o checksum factual final e o GO humano. O eco informa antes da gravação que a ação é terminal, que o histórico interno nasce arquivado e que envio continua separado. A nova API privada `/api/painel-retencao-editorial` lê somente estado seguro de retenção; descarte é ação separada, exige `DESCARTAR HISTORICO`, usa o ator da sessão e só declara sucesso após read-back. A faixa interna mostra retenção apenas em versão fechada/liberada ou enviada. O P5 ganha tradução fail-closed para `fechamento_editorial_pendente`. `verifica:av4`, decisão, envio, AV2, AV3, RA4, revisão, público e build passaram; lint ficou somente com as seis falhas TypeScript herdadas. **No Dácora Reports (`xdvnybvflhmhescjjxip`) já estão aplicadas `20260818213500_av4_fechamento_editorial_retencao` e `20260818214500_av4_hardening_acl_trigger_envio`, com read-back remoto concluído.** Merge em `main`, deploy e validação humana continuam sob GO separado. Handoff canônico: `OpenClaw-Dacora/docs/HANDOFF_AV4_FECHAMENTO_EDITORIAL_2026-08-18.md`.
