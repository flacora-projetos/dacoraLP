...
**AV3 — histórico interno das análises, implementada e validada LOCALMENTE em 2026-08-18; ainda NÃO publicada:** a branch `feat/av3-historico-interno` acrescenta `/api/painel-historico-analises`, protegido por sessão + allow-list e com identidade (`cliente_slug + competencia + versao`) resolvida no servidor a partir do relatório aberto. A faixa da revisão ganhou uma timeline `<details>` recolhida por padrão, agrupada por seção/introdução, mostrando estado, revisor, horário da revisão, checksum factual, coleta de referência e texto/decisão `sem_analise`. Falha da timeline não bloqueia o relatório nem a decisão. A migration correspondente vive no OpenClaw como `0017_av3_historico_coleta_referencia.sql` e ainda não foi aplicada remotamente; antes dela, o endpoint faz fallback honesto com `coletadoEmReferencia: null`, sem fabricar data. `verifica:av3`, `verifica:av2`, `verifica:analise`, `verifica:revisao`, `verifica:decisao`, `verifica:envio`, `verifica:publico` e build passaram. O lint voltou ao baseline de seis erros TypeScript herdados, nenhum AV3. Produção/main permanecem intactas; migration, merge/deploy e smoke autenticado real aguardam GO explícito. Handoff canônico: `OpenClaw-Dacora/docs/HANDOFF_AV3_HISTORICO_INTERNO_2026-08-18.md`.
