...
**AV2 Portal — implementada e validada localmente em branch isolada em 2026-08-18, ainda não integrada/publicada:** `feat/av2-portal-prontidao-aviso` religa a prontidão editorial ao modelo AV por identidade lógica (`cliente_slug + competencia + versao`), mantendo o legado para competências que ainda não passaram pela ponte. `revisao_necessaria` e divergência de checksum factual vencem sempre, inclusive no endpoint de aprovação; falha de leitura do modelo AV continua fail-closed. Aplicar/editar/dispensar passam a registrar também `registrar_revisao_editorial_atual`, e **Contexto do mês** passa a registrar também `salvar_contexto_editorial_relatorio` quando existe impressão digital factual; relatórios legados continuam operando pelo caminho antigo. A bancada avisa que a análise vale até a próxima atualização e usa `fontes[].coletadoEm` do snapshot corrente, nunca o relógio do navegador. `verifica:av2`, `verifica:ra4`, `verifica:analise`, `verifica:decisao`, `verifica:revisao`, `verifica:publico`, `git diff --check` e build passaram. `lint` continua somente com os seis erros TypeScript herdados já registrados, sem erro AV2. **Não há alegação de smoke autenticado desktop/390×844 nesta rodada; esse gate continua aberto antes de publicação.** Nenhuma decisão real, Supabase, merge em `main` ou deploy foi acionado.
