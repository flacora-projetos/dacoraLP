...
**Instagram unificado — implementado em branch isolada em 03/09/2026, fora de produção:** `codex/mensal-instagram-unificado-2026-09-03` amplia o B1 para combinar a faixa do perfil com um funil opcional dentro da mesma seção. A revisão analítica recebe `{ faixa, funil }`, preservando o dado que antes vinha de `analise-instagram`. O teste de contrato renderiza as duas partes e impede duplicação do funil; `verifica:funil`, `verifica:revisao`, lint, prebuild e build completo passaram. Smoke local não mutante passou em **1440×900** e **390×844**, com uma única seção “O perfil do Instagram no mês” e `scrollWidth === clientWidth`. Nenhuma rota real, decisão, intenção de envio ou banco foi alterado. Handoff canônico: `OpenClaw-Dacora/docs/HANDOFF_MENSAL_INSTAGRAM_E_DESTINOS_2026-09-03.md`.
