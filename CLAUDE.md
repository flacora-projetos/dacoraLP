...
A RA2 está em correção após dois testes reais exporem falhas diferentes. `dfbb384` já corrigiu em produção o contraste e o `contexto_indisponivel` dos snapshots anteriores à RA1. Na primeira geração real seguinte, o Sonnet devolveu um envelope textual que o parser JSON estrito recusou como `saida_invalida`. A branch `codex/correcao-ra2-contexto-legado` agora aceita o objeto `{"texto":"..."}` estrito, cercado por Markdown ou após preâmbulo, continua recusando texto livre e mantém a validação numérica factual. `verifica:analise`, `verifica:revisao` e build passaram; esta segunda correção ainda não foi publicada neste marco. O backend continua autenticado, usa service role somente no servidor e chama `claude-sonnet-5` sem tools. Decisão e envio seguem fora. A introdução é o alvo principal da caneta mágica; C1/C2/C3 não deve ser
