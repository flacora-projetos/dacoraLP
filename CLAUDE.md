...
A RA2 está em correção após o primeiro teste autenticado em produção provar dois defeitos: contraste quase ilegível e `contexto_indisponivel` nos relatórios reais anteriores à RA1. A branch `codex/correcao-ra2-contexto-legado` deriva `analysis_context_v1` server-side das faixas já persistidas, sem mutar snapshot/checksum, e apresenta a caneta em cartão claro destacado. O Supabase confirmou 96 relatórios externos/gerados compatíveis; os seis restantes são voz sintética ou mensal interno fora desta caneta. `verifica:analise`, `verifica:revisao` e build passaram; correção ainda não publicada neste marco. O backend continua autenticado, usa service role somente no servidor e chama `claude-sonnet-5` sem tools. Decisão e envio seguem fora. A introdução é o alvo principal da caneta mágica; C1/C2/C3 não deve ser
