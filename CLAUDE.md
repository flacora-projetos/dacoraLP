...
A RA2 publicada em `main/c5e6407` falhou no teste autenticado real com HTTP 422 `saida_invalida`: apesar da remoção do veto numérico, o contrato do provider ainda exigia uma saída aceita pelo parser e mantinha o teto de 3.500 caracteres. A correção está pronta e pushada em `codex/ra2-texto-livre-provider`; `a0c2698` pede texto puro, preserva todos os blocos textuais, deixa JSON como compatibilidade opcional e remove o teto editorial de geração/edição. A sessão coordenadora repetiu `verifica:analise`, `verifica:revisao` e o build completo com sucesso. Ainda não houve merge, deploy nem novo teste autenticado; RA3 continua bloqueada. O backend preserva auth, service role somente no servidor, contexto reconstruído no servidor, checksum e auditoria. Decisão e envio seguem fora. A introdução é o alvo principal da caneta mágica; C1/C2/C3 não deve ser
