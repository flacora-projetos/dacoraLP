...
A RA2 está em nova correção de produto após a regex numérica bloquear a primeira saída já parseada do Sonnet. Decisão do Flávio: o modelo lê o conteúdo disponível do relatório e escreve a proposta; comparação lado a lado e revisão humana substituem a falsa segurança lexical. A branch remove o veto numérico de geração e edição e envia ao modelo introdução, fatos, relações, destaques, atenção, próximos passos, seções, tabelas, evolução mensal, quebras e séries. Assets e paths continuam fora; auth, checksum e auditoria permanecem. `verifica:analise`, `verifica:revisao` e build passaram; ainda não publicado neste marco. O backend usa service role somente no servidor e chama `claude-sonnet-5` sem tools. Decisão e envio seguem fora. A introdução é o alvo principal da caneta mágica; C1/C2/C3 não deve ser
